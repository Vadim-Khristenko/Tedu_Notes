{@html '<!-- NoteLegacy.svelte  -  will be deprecated soon -->'}
<style lang="less">
	.note-layout {
		display: flex;
				...
				{#each list as note (note.id)}
					<article class="note-card" animate:flip={{ duration: 150 }}>
						<header class="note-card__title">
							...
							<a href={getRelativeLocaleUrl(locale, `/note-legacy/${note.id.split("/").slice(1).join("/")}`)}>
...
let filtered: any[] = $derived.by(() => {
	...
	if (initial) {
		...
		const url = getRelativeLocaleUrl(locale, `/note-legacy${search ? `?${search}` : ""}`);
